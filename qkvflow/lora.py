"""
Small modiffication compared to levanter
"""
import dataclasses
import functools
import json
...
class LowRankLinear(eqx.Module):
    ...
    @staticmethod
    def init(
        In: hax.Axis, Out: Axis, r: int, alpha: float, dropout_prob: float, *, key
    ):
        """
        ...
        lora_B = hnn.Linear(weight=weight_B, bias=None, In=_R, Out=Out)
...
def partition_lora_params(params: M) -> Tuple[M, M]:
    ...
    """
    lora_params, base_params = eqx.partition(
        params, is_lora_param, is_leaf=is_lora_param
    )
...
def _loraize(
    ...
    def _batchify_ctor(ctor):
        ...
        return functools.reduce(
            lambda ctor, batch_axis: hax.vmap(ctor, batch_axis),
            reversed(batch_dims),
            ctor,
        )
    ...
    def _loraize_module(module, key_path):
        ...
        if isinstance(module, hnn.Stacked):
            ...
            return _batchify_ctor(LoraLinear.init)(
                module, config.r, config.alpha, config.dropout, key=batched_key
            )
        else:
            return module
...
def merge_lora_modules(module: M) -> M:
    ...
    return jax.tree_util.tree_map(
        _merge_lora_modules, module, is_leaf=lambda node: isinstance(node, LoraLinear)
    )
    ...
    **hf_upload_kwargs,
    ...
    def cb(step: StepInfo):
        ...
        if upload_to_hf is not None and "commit_message" not in hf_upload_kwargs:
            ...
            my_upload_kwargs[
                "commit_message"
            ] = f"Upload for step {step.step} from Levanter"
        else:
    ...
    **hf_upload_kwargs,
    ...
    def save_merged_hf_model_cb(step: StepInfo):
        ...
        save_merged_hf_model(
            model, converter, path, upload_to_hf=upload_to_hf, **my_upload_kwargs
        )
...
def to_hf_config(
    config: LoraConfig, base_model_name_or_path: Optional[str] = None, **kwargs
) -> dict:
    """
