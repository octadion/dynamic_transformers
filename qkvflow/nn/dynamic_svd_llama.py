...
class LlamaAdaptiveTemporalMLP(eqx.Module):
    ...
    act: Callable = eqx.field(static=True)
    gate_logit_gate: hax.NamedArray
    gate_logit_up: hax.NamedArray
    gate_logit_down: hax.NamedArray
    ...
    def __call__(self, time_embed: NamedArray, x: NamedArray, multipliers: Dict[str, NamedArray], *, key=None):
        ...
        gate_gate_raw = jax.nn.sigmoid(self.gate_logit_gate.astype(jnp.float32).array)
        gate_up_raw = jax.nn.sigmoid(self.gate_logit_up.astype(jnp.float32).array)
        gate_down_raw = jax.nn.sigmoid(self.gate_logit_down.astype(jnp.float32).array)

        def combine_weights(gate, w_svd, w_temp):
            w_svd_raw = jnp.swapaxes(w_svd.array, -1, -2)
            w_temp_raw = w_temp.array
            w_eff_raw = gate * w_svd_raw + (1 - gate) * w_temp_raw
            return hax.named(w_eff_raw, w_temp.axes)

        w_gate_eff = combine_weights(gate_gate_raw, w_gate_svd, w_gate_temporal)
        w_up_eff = combine_weights(gate_up_raw, w_up_svd, w_up_temporal)
        w_down_eff = combine_weights(gate_down_raw, w_down_svd, w_down_temporal)
...
class LlamaAdaptiveBlock(eqx.Module):
    ...
    def init(
        ...
        *, 
        ...
        k_attn, k_mlp, k_adapt, k_gate, ln_1_key, ln_2_key = jrandom.split(key, 6)
...
class SVDLlamaOdeTransformer(eqx.Module):
    ...
    def __call__(self, x: NamedArray, attn_mask, *, key=None) -> NamedArray:
        ...
        def make_do_block(layer_idx, layer_key, current_multipliers):
            def do_block(x_in):
                layer_multipliers = {
                    "gate_proj": current_multipliers.get(f"layer_{layer_idx}_gate_proj"),
                    "up_proj": current_multipliers.get(f"layer_{layer_idx}_up_proj"),
                    "down_proj": current_multipliers.get(f"layer_{layer_idx}_down_proj"),
                }
                
                output = self.block(
                    time_embed.take("layers", layer_idx), x_in, attn_mask, 
                    layer_idx, multipliers=layer_multipliers, key=layer_key
                )
        ...
        final_time_embed = time_embed.take("layers", -1)
